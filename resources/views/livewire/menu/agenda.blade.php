    ...
    <div class="containerLeftAgenda">
                    ...
                    @foreach(auth()->user()->subjects as $subject)
                        ...
                        @foreach($subject->commitments as $commitment)
                            <tr>
                                ...
                                <td>
                                <button class="editButton" wire:click="setActiveComponentAgendaL('edit', {{ $commitment->id }})">Editar</button>
        ...
        @endif
            <div class="containerCreate">
                ...
                <form method="POST" action=" {{route('commitment.store')}}">
                    ...
                    <input type="text" name="description_commitment" id="" placeholder="Descrição" value="{{old('description_commitment')}}" >
                    ...
                    @endif
                    <button class="btn" id="submitRegister" type="submit">Criar Compromisso</button>
                </form>
            </div>
        ...
        @elseif ($activeComponentAgendaL == 'edit')
            <form method="POST" action=" {{route('commitment.update', $selected_commitment->id)}}">
                    @csrf
                    @method('PUT')
                    <input type="text" name="name_commitment" id="" placeholder="Nome" value="{{$selected_commitment->name_commitment}}">
                    <input type="text" name="description_commitment" id="" placeholder="Descrição" value="{{$selected_commitment->description_commitment }}" >
                    <select name="subject_commitment">
                    ...
                    @endif
                    <button class="btn" type="submit" id="voltar" wire:click="setActiveComponentAgendaL('create', 0)">Editar Compromisso</button>
                </form>
                <button class="createButton" wire:click=" setActiveComponentAgendaL('create')">Voltar</button>
        @endif
    </div>
        ...
        <tbody class="tbodyTable">
            @foreach(auth()->user()->subjects as $subject)
                <tr>
                    ...
                    <td>
                        <div class="buttons">
                        <button class="editButton" wire:click=" setActiveSubjectAgendaR('edit', {{$subject->id}})">Editar</button>
...
@endif
            <div class="containerCreate">
                ...
                <form method="POST" action="{{route('subject.store')}}">
                    ...
                    <input type="text" name="teacher_subject" id="" placeholder="Professor" value="{{ old('teacher_subject') }}">
                    @if($errors->any())
                        <div class='invalid-feedback'>
                ...
                <button class="btn" id="submitRegister" type="submit">Voltar</button>
            </div>
        @elseif ($activeSubjectAgendaR == 'edit')
            <form method="POST" action=" {{route('subject.update', $selected_subject->id)}}">
                @csrf
                @method('PUT')
                <input type="text" name="name_subject" id="" placeholder="Nome" value="{{$selected_subject->name_subject}}">
                <input type="text" name="teacher_subject" id="" placeholder="Descrição" value="{{$selected_subject->teacher_subject }}" >
                @if($errors->any())
                    <div class='invalid-feedback'>
                        <ul>
                            <li> {{ $errors->first() }}</li>
                ...
                @endif
                <button class="btn" type="submit" id="voltar" wire:click="setActiveSubjectAgendaR('create', 0)">Editar Matéria</button>
            </form>
            <button class="createButton" wire:click=" setActiveSubjectAgendaR('create, 0')">Cancelar</button>
        @endif
    </div>
